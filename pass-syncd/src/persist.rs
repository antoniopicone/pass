//! CSV persistence for the op log — adapted from reading-list-syncd's
//! persist.rs (itself adapted from serverless-sync), to a single fixed file
//! since this daemon only ever holds one vault's dataset. This file doubles
//! as the user-facing "ledger": plain CSV, readable with any spreadsheet
//! tool or text editor — though unlike reading-list-syncd's, the `value`
//! column here is always an opaque base64 blob (nonce ‖ ciphertext,
//! encrypted one layer up in `passlib::sync` before it ever reaches this
//! daemon), not human-readable plaintext. That's deliberate: this ledger
//! holds password data, so it must stay meaningless without the vault's
//! master password even if a device is compromised at rest.
//!
...
/// `~/.pass-syncd` — a hidden per-user directory holding the ledger CSV
/// plus the `port` sidecar file (see `port_file_path`) that lets clients
/// find a `serve` daemon bound to a non-default port without needing
/// matching CLI flags. Falls back to the working directory if somehow
...
/// doesn't already know it (see `passlib::sync`'s own copy of this same
/// path convention, used when `PASS_SYNCD_URL` isn't set) can find it
/// without being told `--port` directly.
...
/// `main.rs`) and reused across restarts — a device that changed its own
/// id on every restart would look, from every peer's perspective, like an
/// endless stream of brand new devices, each starting the anti-entropy
