//! Adapted from github.com/antoniopicone/serverless-sync (commit 03eced8,
//! src/discovery.rs) by way of `reading-list-syncd` — dataset-agnostic (a
//! peer is just an address), so no logic changes needed for this
//! single-dataset daemon either. The only divergence from a verbatim copy
//! is `LAN_DISCOVERY_PORT`: `reading-list-syncd` and `pass-syncd` are
//! expected to run on the very same machine (this user's own devices), so
//! reusing `reading-list-syncd`'s port would mean both daemons' LAN
//! broadcasts land in each other's `PexCache` and the anti-entropy loop
//! wastes rounds trying to speak this daemon's protocol to the other one's
//! port (harmless — every such round just fails to parse and is dropped —
//! but pure noise). Upstream's multi-application `AGENT_PORT`/`NodeInfo`/
//! `ServiceInfo` (this daemon hosts exactly one dataset, with its own
//! single-dataset `NodeInfo` in main.rs instead) were dropped rather than
//! kept as unused dead weight. Keep in sync by hand if serverless-sync's
//! discovery.rs ever changes; there's no build-time dependency on that
//! repo from here.
//!
...
/// Port used for the LAN broadcast announce/listen (distinct from the
/// sync/control port, and from reading-list-syncd's own 47188, so
/// co-located daemons on the same host don't cross-talk — see the module
/// doc comment).
pub const LAN_DISCOVERY_PORT: u16 = 47288;
...
fn bind_discovery_socket() -> std::io::Result<UdpSocket> {
    ...
    // same host (e.g. running several devices locally without Docker) can
    // each bind the same discovery port instead of only the first one
    // winning it — and, on Linux/BSD, so a broadcast is delivered to every
    // one of them rather than load-balanced to just one.
