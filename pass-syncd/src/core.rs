//! Copied from github.com/antoniopicone/serverless-sync (commit 03eced8,
//! src/core.rs) by way of karakeep-browser-extension's `reading-list-syncd`
//! — its own README documents this file as the intended reuse point ("Copy
//! core.rs into your crate ... and rewrite the two functions where your
//! actual data model lives"). This daemon's entity/value model (a password
//! entry's UUID as `entity`, an opaque encrypted-and-base64 blob as
//! `value`) already fits the generic LWW register below with no changes
//! needed, so `apply`/`local_change` are untouched too — this daemon never
//! sees a password in the clear (see `../README.md`'s "Encryption" section;
//! encryption/decryption happens one layer up, in `passlib::sync`, before a
//! value ever reaches here). Keep this in sync by hand if serverless-sync's
//! core.rs ever changes; there's no build-time dependency on that repo from
//! here.
//!
...
/// One independent CRDT dataset. `main.rs` owns exactly one `Replica` (this
/// daemon hosts a single vault's op log, not a multi-application registry —
/// see the module doc comment in `main.rs`).
pub struct Replica {
    pub device: String,
    seq: u64,
    hlc_time: u64,
...
impl Replica {
    ...
    pub fn apply(&mut self, op: Op) -> bool {
        ...
        // FIX (not present upstream): when the op being applied is one of
        // THIS device's own — the exact case when persist.rs replays a
        // device's past ops back through `apply` at startup, since that's
        // the only entry point persist::load's replay uses — `self.seq`
        // (the counter `local_change` mints the *next* local op from) must
        // be bumped too. Left unfixed, a restart silently resets `seq` to 0
        // while `vv[device]` correctly reflects the true high-water mark;
        // the first local write after that then reuses an already-used seq
        // number and regresses `vv[device]`, which a peer that already
        // synced past the real value will read as a stale duplicate and
        // silently drop — real data loss. `local_change` still does its own
        // `self.seq += 1`, so this only ever raises the floor, never
        // invents a value.
    ...
    /// value). This is the signal used to decide whether replicas are
    /// aligned: two replicas with the same state must always produce the
    /// same fingerprint, regardless of the order in which they applied
    /// their ops. Note the "value" here is still the encrypted blob (this
    /// module never decrypts anything), so the fingerprint proves two
    /// devices agree without this daemon ever learning what they agree on.
